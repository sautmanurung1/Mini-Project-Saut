...
func TestGetAssignmentByIdService(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		assignmentService.On("GetAssignmentByIdService", mock.Anything, mock.Anything).Return("Success Get Assignment By Id", nil).Once()

		assignmentService := domains.AssignmentService(assignmentService)
		assign, err := assignmentService.GetAssignmentByIdService(int(assignmentData.ID), assignmentData)

		assert.Equal(t, assign, "Success Get Assignment By Id")
		assert.NoError(t, err)
	})

	t.Run("Failed", func(t *testing.T) {
		assignmentService.On("GetAssignmentByIdService", mock.Anything, mock.Anything).Return("Failed To Get Assignment By Id", errors.New("Error To Make Unit Testing")).Once()

		assignmentService := domains.AssignmentService(assignmentService)
		assign, err := assignmentService.GetAssignmentByIdService(int(assignmentData.ID), assignmentData)

		assert.Equal(t, assign, "Failed To Get Assignment By Id")
		assert.Error(t, err)
	})
}
...
func TestUpdateAssignmentService(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		assignmentService.On("UpdateAssignmentService", mock.Anything, mock.Anything).Return("Success Update Assignment", nil).Once()

		assignmentService := domains.AssignmentService(assignmentService)
		assign, err := assignmentService.UpdateAssignmentService(int(assignmentData.ID), assignmentData)

		assert.Equal(t, assign, "Success Update Assignment")
		assert.NoError(t, err)
	})

	t.Run("Failed", func(t *testing.T) {
		assignmentService.On("UpdateAssignmentService", mock.Anything, mock.Anything).Return("Failed Update Assignment", errors.New("Error To Make Unit Testing")).Once()

		assignmentService := domains.AssignmentService(assignmentService)
		assign, err := assignmentService.UpdateAssignmentService(int(assignmentData.ID), assignmentData)

		assert.Equal(t, assign, "Failed Update Assignment")
		assert.Error(t, err)
	})
}

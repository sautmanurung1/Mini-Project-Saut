...
func TestGetQuestionByIdService(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		questionService.On("GetQuestionByIDService", mock.Anything, mock.Anything).Return("Success Get Question By Id", nil).Once()

		questionService := domains.QuestionService(questionService)
		question, err := questionService.GetQuestionByIDService(int(questionData.ID), questionData)

		assert.Equal(t, question, "Success Get Question By Id")
		assert.NoError(t, err)
	})

	t.Run("Failed", func(t *testing.T) {
		questionService.On("GetQuestionByIDService", mock.Anything, mock.Anything).Return("Failed Get Question By Id", errors.New("Error To Make Unit Testing")).Once()

		questionService := domains.QuestionService(questionService)
		question, err := questionService.GetQuestionByIDService(int(questionData.ID), questionData)

		assert.Equal(t, question, "Failed Get Question By Id")
		assert.Error(t, err)
	})
}
...
func TestUpdateQuestionService(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		questionService.On("UpdateQuestionService", mock.Anything, mock.Anything).Return("Success Update Question", nil).Once()

		questionService := domains.QuestionService(questionService)
		question, err := questionService.UpdateQuestionService(int(questionData.ID), questionData)

		assert.Equal(t, question, "Success Update Question")
		assert.NoError(t, err)
	})

	t.Run("Failed", func(t *testing.T) {
		questionService.On("UpdateQuestionService", mock.Anything, mock.Anything).Return("Failed Update Question", errors.New("Error To Make Unit Testing")).Once()

		questionService := domains.QuestionService(questionService)
		question, err := questionService.UpdateQuestionService(int(questionData.ID), questionData)

		assert.Equal(t, question, "Failed Update Question")
		assert.Error(t, err)
	})
}
